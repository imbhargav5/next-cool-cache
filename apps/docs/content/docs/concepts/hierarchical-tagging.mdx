...
One of the most powerful features of `next-typed-cache` is automatic hierarchical tag registration. This enables you to invalidate a parent and have all children automatically refresh.
...
With `next-typed-cache`, calling `cacheTag()` automatically registers the tag AND all its ancestors:
...
Under the hood, `next-typed-cache` uses these utility functions:
